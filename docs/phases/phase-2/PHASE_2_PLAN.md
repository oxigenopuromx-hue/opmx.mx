...
- **Fase 1: implementación local completa; cierre formal pendiente de staging.** Todos los criterios de aceptación verificables localmente están satisfechos, pero **Release 1.0 ("Portal Fundacional") todavía no está formalmente cerrado** — su propio criterio de salida "staging verificable" no se cumple porque no existe staging desplegado (`PHASE_1_REPORT.md` §9–§11; `STAGING.md`; `OPMX_PRODUCT_ROADMAP_v1.0.md` §7). No bloqueante para el alcance de planeación de este documento (§4).
...
| ------------------------------------------------------------------------- | --------------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Fase 1: implementación local completa; cierre formal pendiente de staging | ✅ local / ❌ Release 1.0 no cerrado formalmente          | No                      | No                                                                                                                                       |
...
| `isReal`                                         | booleano explícito                               | **Obligatorio.** `false` para cualquier fixture o plantilla; solo `true` para un estudio real aprobado (Release 1.3). Es una **salvaguarda de integridad editorial/técnica**, no una frontera de seguridad por sí sola — debe combinarse con las medidas de defensa en profundidad de §16 para todo contenido con `isReal: false`. |
...
- **`CitationBlock`** — genera cita corta y cita académica (`MASTER_SPEC` §60) **exclusivamente formateando** información bibliográfica ya existente y validada dentro del objeto `Study` (institución, responsables, patrocinador, fecha, metodología, URL, versión, título y demás datos bibliográficos). **No debe inventar, inferir, completar ni generar automáticamente ninguno de esos campos si faltan.** Si un campo requerido para una modalidad de cita concreta (corta o académica) no está presente y validado en el `Study`, `CitationBlock` debe **fallar validación o simplemente no ofrecer esa modalidad de cita** — el diseño final de implementación decide cuál de las dos, pero en ningún caso debe producir una cita con datos faltantes rellenados por inferencia. Se construye y prueba ahora, con fixtures sintéticos completos e incompletos (§17), aunque no haya un estudio real que lo use todavía, precisamente para que el primer estudio real (Release 1.3) no requiera diseñarlo desde cero.
...
- El campo `isReal: false` obligatorio en todo fixture (§9) es una **salvaguarda de integridad editorial/técnica**, no una frontera de seguridad: por sí solo no impide que un componente mal reutilizado exponga contenido sintético como si fuera real. Por eso, para todo contenido con `isReal: false`, la futura implementación debe aplicar **defensa en profundidad**, no depender de un solo campo:
  - `noindex` en la metadata de la página.
  - Exclusión explícita de `sitemap.xml`.
  - Ausencia total de JSON-LD `Dataset`/`Article` (§14).
  - Badge visible e inequívoco en la interfaz — p. ej. "DATOS SINTÉTICOS" — no un aviso sutil ni solo un campo interno.
  - Validación automatizada mediante pruebas (E2E que confirmen `noindex`, ausencia de la ruta en el sitemap, ausencia de JSON-LD y presencia del badge — ver §17).
...
| Tipo                              | Cobertura                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unitarias                         | Validación del esquema `Study` (casos válidos e inválidos, incluyendo `isReal` mal usado); integridad del modelo electoral (candidaturas múltiples sin sobrescritura); `CitationBlock` con fixture bibliográfico completo (genera la cita) y con fixture incompleto (falla validación o no ofrece esa modalidad — nunca rellena el campo faltante) |
...
**La Fase 2 no puede declararse cerrada mientras las verificaciones sobre staging sigan pendientes.** No existe un estado intermedio de "Fase 2 técnicamente completa, staging pendiente" — mientras la repetición de pruebas sobre staging (paso 5 abajo) no se haya ejecutado y documentado, Fase 2 permanece **en implementación**, no en cierre. Esto aplica el mismo estándar que este documento ya exige retroactivamente para el propio Release 1.0 (§3): completar el trabajo local no equivale a cerrar formalmente la fase.

La secuencia es la siguiente, y ningún paso puede omitirse ni invertirse:

1. **Staging validado** — ADR-001 en estado `Accepted`, staging desplegado y verificado (`docs/adr/ADR-001-hosting.md`; §26 de este documento).
2. **Autorización explícita de implementación** de Fase 2, separada de la autorización de planeación que dio origen a este documento.
3. **Implementación de Fase 2** — los 12 pasos de §20, cada uno con su commit y su verificación local.
4. **Pruebas locales** — criterios de §21 verificados en build local (Performance ≥ 90, Accessibility ≥ 95, Best Practices ≥ 95, SEO ≥ 95, LCP < 2.5 s, CLS < 0.1), igual metodología que Fase 1.
5. **Pruebas sobre staging** — repetición, como mínimo, de **Lighthouse/Core Web Vitals, accesibilidad (axe + verificación manual de teclado), SEO técnico (robots/sitemap/canonical) y disponibilidad**, contra la URL real de staging, no contra un build local.
6. **Cierre formal de Fase 2** — únicamente después de que el paso 5 esté completo y documentado con comandos y resultados en `PHASE_2_REPORT.md`.

Condiciones adicionales para el cierre formal (paso 6), sin alterar la secuencia anterior:

- Todos los criterios de §21 verificados con comandos y evidencia, tanto locales (paso 4) como de staging (paso 5).
- `ARCHITECTURE.md` refleja el estado real del código.
- Los tres borradores institucionales de §18 existen, con el encabezado DRAFT, sin presentarse como aprobados.

Fase 2 no requiere, para su cierre formal, que exista un estudio real publicado ni que los borradores institucionales estén aprobados institucionalmente — eso corresponde a Release 1.3 y a decisiones fuera de este alcance. Pero sí requiere, sin excepción, que el paso 5 se haya completado — no puede sustituirse ni omitirse por falta de staging.
